...
def evaluate(args, model, tokenizer, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            decoder_predictions = model.generate(
                **inputs,
                min_length=args.max_seq_length,
                max_length=args.max_seq_length,
                decoder_start_token_id=model.config.decoder.pad_token_id)
            logger.error("output is %s", decoder_predictions.shape)
        nb_eval_steps += 1

        decoder_predictions = decoder_predictions.detach().cpu().numpy()

        if preds is None:
            preds = decoder_predictions
            out_ids = output_ids.detach().cpu().numpy()
        else:
            preds = np.append(preds, decoder_predictions, axis=0)
            out_ids = np.append(out_ids,
    ...
    logger.error("Out_ids is %s", out_ids.shape)
    logger.error("preds is %s", preds.shape)
    results = {
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    all_input_ids = torch.tensor([feat.input_ids for feat in features],
                                 dtype=torch.long)
    all_output_ids = torch.tensor([feat.output_ids for feat in features],
                                  dtype=torch.long)
    all_input_mask = torch.tensor([feat.input_mask for feat in features],
                                  dtype=torch.long)
    all_output_mask = torch.tensor([feat.output_mask for feat in features],
                                   dtype=torch.long)
    all_segment_ids = torch.tensor([feat.segment_ids for feat in features],
                                   dtype=torch.long)
...
def main():
    ...
    encoder_config = AutoConfig.from_pretrained(
        args.config_name if args.config_name else
        (args.model_name_or_path +
         ("/encoder" if os.path.exists(args.model_name_or_path) else "")),
        cache_dir=args.cache_dir if args.cache_dir else None,
    )
    decoder_config = AutoConfig.from_pretrained(
        args.config_name if args.config_name else
        (args.model_name_or_path +
         ("/decoder" if os.path.exists(args.model_name_or_path) else "")),
        cache_dir=args.cache_dir if args.cache_dir else None,
    )
    ...
    if os.path.exists(os.path.join(
            args.model_name_or_path, "encoder")) and os.path.exists(
