...
def train(args, train_dataset, model, tokenizer, pad_token_label_id):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"encoder_input_ids": batch[0], "decoder_input_ids": batch[1]}
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    if (
                        ...
                        results, _ = evaluate(
                            args, model, tokenizer, pad_token_label_id, mode="dev"
                        )
...
def evaluate(args, model, tokenizer, pad_token_label_id, mode, prefix=""):
    eval_dataset = load_and_cache_examples(
        args, tokenizer, pad_token_label_id, mode=mode
    )
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if preds is None:
            ...
            out_ids = inputs["decoder_input_ids"].detach().cpu().numpy()
        else:
            preds = np.append(
                preds, decoder_predictions.argmax(-1).detach().cpu().numpy(), axis=0
            )
            out_ids = np.append(
                out_ids, inputs["decoder_input_ids"].detach().cpu().numpy(), axis=0
            )
    ...
    results = {
        ...
        "precision": precision_score(
            out_ids.flatten(), preds.flatten(), average="micro"
        ),
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    all_encoder_input_ids = torch.tensor(
        [feat.encoder_input_ids for feat in features], dtype=torch.long
    )
    all_decoder_input_ids = torch.tensor(
        [feat.decoder_input_ids for feat in features], dtype=torch.long
    )
    all_input_mask = torch.tensor(
        [feat.input_mask for feat in features], dtype=torch.long
    )
    all_output_mask = torch.tensor(
        [feat.output_mask for feat in features], dtype=torch.long
    )
    all_segment_ids = torch.tensor(
        [feat.segment_ids for feat in features], dtype=torch.long
    )
...
def main():
    ...
    if os.path.exists(
        os.path.join(args.model_name_or_path, "encoder")
    ) and os.path.exists(os.path.join(args.model_name_or_path), "decoder"):
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        result, predictions = evaluate(
            args, model, tokenizer, pad_token_label_id, mode="test"
        )
        # Save results
