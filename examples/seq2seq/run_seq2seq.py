...
def train(args, train_dataset, model, tokenizer, pad_token_label_id):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = loss_fct(decoder_predictions.view(-1, vocab_size), inputs["decoder_input_ids"].view(-1),)
...
def evaluate(args, model, tokenizer, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if preds is None:
            ...
            out_ids = np.append(out_ids, inputs["decoder_input_ids"].detach().cpu().numpy(), axis=0)
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    all_encoder_input_ids = torch.tensor([feat.encoder_input_ids for feat in features], dtype=torch.long)
    all_decoder_input_ids = torch.tensor([feat.decoder_input_ids for feat in features], dtype=torch.long)
    ...
    dataset = TensorDataset(
        all_encoder_input_ids, all_decoder_input_ids, all_input_mask, all_output_mask, all_segment_ids,
    )
    return dataset
