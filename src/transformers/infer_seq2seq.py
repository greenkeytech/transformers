...
output_dir="date_gpt2"
cache_dir=None
tokenizer_args={}
config = AutoConfig.from_pretrained(os.path.join(output_dir, "tokenizer_config.json"))
tokenizer = AutoTokenizer.from_pretrained(output_dir, config=config, **tokenizer_args)

model_name_or_path="date_gpt2"
...
def apply_seq2seq(input_text, tokenizer, model):
    """
    Given input text, tokenizer, and model,
    return output text
    
    CPU invocation takes 200 ms
    """
    tokens = tokenizer.tokenize(eg)
    max_seq_length=model.encoder.config.max_length 
    pad_token=tokenizer.pad_token
    input_ids = tokenizer.convert_tokens_to_ids(tokens)
    input_padding_length = max_seq_length - len(input_ids)
    input_ids += [tokenizer.pad_token_id] * input_padding_length
    eval_dataset = torch.tensor(input_ids).unsqueeze(0)
    model.eval()
    output = model(input_ids=eval_dataset)
    decoder_predictions = output[0]
    return tokenizer.decode(decoder_predictions[0].argmax(-1), skip_special_tokens=True, clean_up_tokenization_spaces=True,)
...
def evaluate(args, model, tokenizer, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        input_ids, output_ids, _, output_mask, _ = batch
        with torch.no_grad():
            ...
            # See the PreTrainedEncoderDecoder class for more details
            output = model(**inputs)
            decoder_predictions = output[0]
            vocab_size = decoder_predictions.shape[-1]

            # only compute the loss on the output where not padded
            ...
            # non_pad_indx = torch.nonzero(output_mask)
            # loss = loss_fct(
            #         decoder_predictions.view(-1, vocab_size)[non_pad_indx].view(-1,vocab_size), 
            #         output_ids.view(-1)[non_pad_indx].view(-1),
            #        )
    ...
    results = {
        "loss": eval_loss,
        "precision": precision_score(out_ids.flatten(), preds.flatten(), average="micro"),
        "recall": recall_score(out_ids.flatten(), preds.flatten(), average="micro"),
        "f1": f1_score(out_ids.flatten(), preds.flatten(), average="micro"),
    }
